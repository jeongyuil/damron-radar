...
- 시드: 이슈 31·엔티티 227 (`data/seeds/` — 엔티티는 v1, 2026-09-09 병합. 프롬프트 v1은 v0 참조 중 → v2에서 v1로 전환)
- 사전: `data/dictionaries/` (2주차, STT 정정·잡음·판례집·부호 규칙·어휘 — README에 소비자 명시)
...
골든셋 선발전: `caffeinate -i uv run python -u scripts/golden_bench.py` (후보 모델 type/sentiment/stance 채점 → `reports/golden_bench_<vid>_<labeler>.md`)
